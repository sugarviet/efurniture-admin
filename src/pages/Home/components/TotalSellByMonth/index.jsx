...
const TotalSellByMonth = () => {
  const { handleChangeRangeDate, selectOptionsDateRange } = useManageTotalSell();

  // Generate fake revenue data for demonstration
  const generateFakeRevenueData = () => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const profit = Math.floor(Math.random() * 10000); // Random profit value
      revenueData.push({ date: month, profit });
    }
    return revenueData;
  };

  const revenueData = generateFakeRevenueData();
